import { BROKER_SETTINGS_FOLDER_NAVIGATION_MENU_ITEM_ID } from 'src/navigation-menu-items/broker-settings-folder.navigation-menu-item';
import { DEAL_CHECKLISTS_VIEW_ID } from 'src/views/deal-checklists.view';
import {
  NavigationMenuItemType,
  defineNavigationMenuItem,
} from 'twenty-sdk/define';

export default defineNavigationMenuItem({
  universalIdentifier: '6b6d0000-3100-4000-8000-000000000013',
  name: 'Deal checklists',
  icon: 'IconChecklist',
  position: 42,
  type: NavigationMenuItemType.VIEW,
  viewUniversalIdentifier: DEAL_CHECKLISTS_VIEW_ID,
  folderUniversalIdentifier: BROKER_SETTINGS_FOLDER_NAVIGATION_MENU_ITEM_ID,
});
