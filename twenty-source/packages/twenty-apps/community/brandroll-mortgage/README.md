...
- The opened loan workspace now uses saved, resizable panels: the left loan workflow menu defaults to `260px`, the right tools workspace defaults to `430px`, both can be dragged wider/narrower on desktop, and preferences are stored in browser local storage under `brokerapp.loanWorkspace.layout.v1`.
...
- LoanDox is designed to store document metadata and external references only. Paperless-ngx is the preferred v1 document archive/OCR backend once configured; BrokerApp stores Paperless document IDs, source, OCR status, AI review status and visibility flags rather than raw sensitive files.
